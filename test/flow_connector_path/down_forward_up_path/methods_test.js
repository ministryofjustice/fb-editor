...
describe("DownForwardUpPath", function() {
  ...
  const CONTAINER_ID = "downforwarduppath-for-testing-methods-container";
  const COMPONENT_ID = "downforwarduppath-for-testing-methods-connector";

  describe("Methods", function() {
    var created;
    var expectedPathValue = "M 701,125 v428 a10,10 0 0 0 10,10 h505 a10,10 0 0 0 10,-10 v-480 a10,10 0 0 1 10,-10 h14";
    const POINTS = {
                     from_x: 701,
                     from_y: 125,
                     to_x: 1250,
    ...
    it("should build the $node", function() {
      ...
      expect(created.connector.$node.hasClass("DownForwardUpPath")).to.be.true;
    });
    ...
    /* TEST METHOD: lines()
     * The DownForwardUpPath class produces three lines;
     ...
     * DownForwardUpPath shoule be able to move the vertical line for overlap
     ...
     * DownForwardUpPath only has one vertical line that can move.
     **/
    it("should return true if overlap was fixed (found)", function() {
      var clashingPath = helpers.createFlowConnectorPath('DownForwardUpPath', COMPONENT_ID + "-overlap", POINTS, {
        container: $("#" + CONTAINER_ID),
        from: c.FAKE_FLOW_ITEM_1,
        to: c.FAKE_FLOW_ITEM_2,
        top: 5,
