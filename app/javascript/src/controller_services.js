...
ServicesController.edit = function () {
  ...
  const flowLayoutTime = performance.measure(
    "flow-layout-time",
    "flow-layout-start",
    "flow-layout-end",
  );
  const flowItemPositionTime = performance.measure(
    "flow-item-position-time",
    "flow-layout-start",
    "flow-items-positioned",
  );
  const flowConnectorsTime = performance.measure(
    "flow-connectors-time",
    "flow-connectors-start",
    "flow-connectors-end",
  );
  const flowConnectorAdjustmentsTime = performance.measure(
    "flow-connector-adjust-time",
    "flow-connector-adjustments-start",
    "flow-connector-adjustments-end",
  );
};
    ...
    $.get(url)
      ...
      .fail((response) => {
        ...
        view.dialog.open({
          ...
          content: action
            ? view.text.dialogs.undo_redo_error[action]?.content
            : view.text.dialogs.undo_redo_error.content,
        });
      });
  });
}
...
function addEventListeners() {
  ...
  document.body.addEventListener("click", function (event) {
    ...
    if (
      !(
        event.target.closest(".flow-thumbnail") ||
        event.target.closest(".govuk-link")
      )
    )
      return;
...
function adjustBranchConditionPositions($overview) {
  var strokeWidth =
    $(".FlowConnectorPath path").eq(0).css("stroke-width") || "";
...
function adjustOverviewHeight($overview) {
  ...
  var $firstRowConditions = $(
    SELECTOR_FLOW_CONDITION + '[data-row="0"]',
    $overview,
  );
  ...
  $firstRowConditions.each(function () {
    var top =
      FLOW_GRID_ROW_HEIGHT / 4 - $(this).find(".flow-expressions").height();
    topNumbers.push(top);
  });
  ...
  $paths.each(function () {
    ...
    var top = vLines.reduce((min, line) => {
      return line.range.start < min ? line.range.start : min;
    }, 0);
    // Add the highest ending point of a line to the bottom numbers array
    var bottom = vLines.reduce((max, line) => {
      return line.range.end > max ? line.range.end : max;
    }, 0);
...
function adjustOverviewWidth($overview) {
  var $items = $(
    [SELECTOR_FLOW_ITEM, SELECTOR_FLOW_CONDITION, SELECTOR_FLOW_LINE].join(
      ", ",
    ),
    $overview,
  );
...
function addServicesContentScrollContainer(view) {
  ...
  adjustScrollDimensionsAndPositions(
    $body,
    $container,
    $main,
    $header,
 ...
 **/
function adjustScrollDimensionsAndPositions(
  $body,
  $container,
  $main,
  $header,
  ...
  if ($body.outerHeight() < window.innerHeight) {
    $footer.height(
      $footer.height() + (window.innerHeight - $body.outerHeight()),
    );
  }
...
function applyPageFlowConnectorPaths(view, $overview) {
  var $items = $overview.find(
    '.flow-page[data-next]:not([data-next="trailing-route"])',
  );
...
function applyBranchFlowConnectorPaths(view, $overview) {
  ...
  $flowItemElements.filter(SELECTOR_FLOW_BRANCH).each(function () {
    ...
    var branchY =
      $branch.data("instance").position.top + FLOW_GRID_ROW_HEIGHT / 2;
    ...
    $conditions.each(function (index) {
      ...
      var $destination = $(
        '[data-fb-id="' + $condition.data("next") + '"]',
        $overview,
      );
      ...
      var conditionY =
        $condition.data("row") * FLOW_GRID_ROW_HEIGHT +
        FLOW_GRID_ROW_HEIGHT / 4;
...
function renderFlowConnectorPaths($container) {
  $container.data("paths").forEach((path) => {
    path.render();
  });
}
...
function adjustOverlappingFlowConnectorPaths($container) {
  ...
  var paths = $container
    .data("paths")
    .filter((path) => !["ForwardPath", "DownForwardPath"].includes(path.type));
  var somethingMoved;
  ...
  do {
    ...
    if (loopCount >= recursionLimit) {
      console.error(
        "Oops! Somethign may have gone wrong. The overlap loop has gone round %d times and tripped the limit.",
        recursionLimit,
      );
    }
...
function calculateAndCreatePageFlowConnectorPath(points, config) {
  ...
  var destinationInNextColumn =
    utilities.difference(columnItem, columnNext) == 1;
  var path;
